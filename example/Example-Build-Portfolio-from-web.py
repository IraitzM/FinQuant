...
# # Building a portfolio with data from `yfinance`
# ## Building a portfolio with `build_portfolio()` by downloading relevant data through `yfinance` with stock names, start and end date and column labels
# This example only focuses on how to use `build_portfolio()` to get an instance of `Portfolio` by providing a few items of information that is passed on to `yfinance`. For a more exhaustive description of this package and example, please try `Example-Analysis` and `Example-Optimisation`.
...
# ## Get data from `yfinance` and build portfolio
...
# ### User friendly interface to yfinance
# As mentioned above, in this example `build_portfolio()` is used to build a portfolio by performing a query to `yfinance`.
#
...
# While yfinance will download lots of different prices for each stock,
...
# As mentioned above, this example only shows how to use `build_portfolio()` to get an instance of `Portfolio` by downloading data through `yfinance`.
